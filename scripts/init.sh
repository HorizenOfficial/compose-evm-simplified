...
select role_value in rpc forger migration; do
...
if ! [ -f "${ENV_FILE}" ]; then
  ...
  if [[ "${role_value}" =~ ^(forger|migration)$ ]]; then
...
if [[ "${role_value}" =~ ^(forger|migration)$ ]]; then
  ...
  echo -e "\n\033[1m=== Project has been initialized correctly for role: ${role_value} and network: ${network_value} ===\033[0m"
