...
echo -e "\n\033[1m=== Creating a backup of the .env file ===\033[0m"
...
# Read the .env.template file line by line, skip blanc lines and comments, store each of the other lines in an array
echo -e "\n\033[1m=== Reading .env.template file ===\033[0m"
...
echo -e "\n\033[1m=== Appending new env vars to .env file ===\033[0m"
  ...
  for item in "${conditional_update_vars[@]}"; do
    if [[ "$item" == "$var_name" ]]; then
      if ! grep -q "^$line" "$ENV_FILE"; then
        echo -e "\nThe value of $var_name in the .env file is different from the value in the .env.template file."
        echo -e "Current value: \033[1m$(grep "^$var_name=" "$ENV_FILE")\033[0m"
        echo -e "New value: \033[1m$line\033[0m\n"
        read -rp "Would you like to update the value of $var_name in the .env file to the value from the .env.template file? ('yes' or 'no') " answer
...
echo ".env upgrade completed successfully"

echo "Please review the changes in the .env file, if there is anything wrong you can restore the backup file .env.bk"

echo "Run cd deployments/${role_value}/${network_value} && docker-compose down && docker-compose up -d to apply the changes."
