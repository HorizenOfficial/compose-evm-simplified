...
echo "Available space in ${seed_dir}: ${seed_dir_available_space_bytes} bytes" | tee -a "${seed_dir}/seed.log"

if [ "${seed_dir_available_space_bytes}" -lt ${extended_content_length} ]; then
  echo "Error: Not enough space in ${seed_dir} to run the zend node." | tee -a "${seed_dir}/seed.log"
  exit 1
fi

if [ "${seed_dir_available_space_bytes}" -ge ${total_size} ]; then
  echo "There is enough storage to download and extract the seed file." | tee -a "${seed_dir}/seed.log"
  echo "Retrieving (aria2): ${remote_file}" | tee -a "${seed_dir}/seed.log"
  ...
  echo "Removing seed file..." | tee -a "${seed_dir}/seed.log"

  rm -f "${seed_dir}/${filename}"

else
  echo "There is no enough storage to download and extract the seed file. Extracting directly from remote." | tee -a "${seed_dir}/seed.log"
  retries=3

  # Extraction loop
  for ((i = 1; i <= retries; i++)); do
    echo "Seed file extraction attempt $i of $retries" | tee -a "${seed_dir}/seed.log"
  ...
  echo "Seed process succeeded. The node will start syncing from the imported data." | tee -a "${seed_dir}/seed.log"
fi

echo "Seed process completed successfully. Exiting script." | tee -a "${seed_dir}/seed.log"
